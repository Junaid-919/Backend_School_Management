...
from ..serializers.disciplinary_records_serializers import DisciplinaryRecordSerializer
from rest_framework.pagination import PageNumberPagination


@api_view(['GET'])
def get_disciplinary_record_data(request):
    try:
        dietplan = DisciplinaryRecord.objects.all()
        serializer = DisciplinaryRecordSerializer(dietplan, many=True)
        return Response(serializer.data)
    except DisciplinaryRecord.DoesNotExist:
        return Response({"error": "DisciplinaryRecord not found."}, status=404)
    
@api_view(['GET'])
def get_disciplinary_record_withid(request, disciplinary_record_id):
    """
    Fetch the appointment of a logged-in Teacher.
    """
    try:
        appointment = DisciplinaryRecord.objects.get(id=disciplinary_record_id)
        serializer = DisciplinaryRecordSerializer(appointment,partial=True)
        return Response(serializer.data)
    except DisciplinaryRecord.DoesNotExist:
        return Response({"error": "DisciplinaryRecord not found."}, status=404)
    
@api_view(['POST'])
def create_disciplinary_record_data(request):
    print("sjhfsfg = ", request.user)
    try:
        ...
        serializer = DisciplinaryRecordSerializer(data=request_data)
...
@api_view(['PUT'])
def update_disciplinary_record_data(request, disciplinary_record_id):
    """
    ...
    try:
        ...
        request_data = request.data.copy()
        print("request_data: ", request_data)
        request_data['disciplinary_record_id'] = DisciplinaryRecord.id

        # Fetch the specific appointment to update
        dietplan = DisciplinaryRecord.objects.get(id=disciplinary_record_id)

        # Serialize and validate the new data
        serializer = DisciplinaryRecordSerializer(dietplan, data=request.data, partial=True)
        if serializer.is_valid():
...
@api_view(['DELETE'])
def delete_disciplinary_record_data(request, disciplinary_record_id):
    try:
        if request.user.role not in ('AD'):
            return Response({"error": "Not Allowed to create attendence."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        dietplan.delete()
        return Response({"message": "DisciplinaryRecord deleted successfully."}, status=204)  # HTTP 204 No Content
