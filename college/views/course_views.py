...
#     try:
#         dietplan = Course.objects.all()
#         serializer = CourseSerializer(dietplan, many=True)
#         return Response(serializer.data)
#     except Course.DoesNotExist:
#         return Response({"error": "Course not found."}, status=404)
    
# @api_view(['GET'])
# def get_course_withid(request, course_id):
#     """
#     Fetch the appointment of a logged-in Teacher.
#     """
#     try:
#         appointment = Course.objects.get(id=course_id)
#         serializer = CourseSerializer(appointment,partial=True)
#         return Response(serializer.data)
#     except Course.DoesNotExist:
#         return Response({"error": "Course not found."}, status=404)
    
# @api_view(['POST'])
# def create_course_data(request):
#     try:
...
# @api_view(['PUT'])
# def update_course_data(request, course_id):
#     """
...
#         dietplan = Course.objects.get(id=course_id)

#         # Serialize and validate the new data
#         serializer = CourseSerializer(dietplan, data=request.data, partial=True)
#         if serializer.is_valid():
...
# @api_view(['DELETE'])
# def delete_course_data(request, course_id):
#     try:
#         if request.user.role not in ('AD'):
#             return Response({"error": "Not Allowed to create attendence."}, status=status.HTTP_400_BAD_REQUEST)
...
#         dietplan.delete()
#         return Response({"message": "Course deleted successfully."}, status=204)  # HTTP 204 No Content
